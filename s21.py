...
def capture_thread():
    ...
    while True:
        ...
        full_frame_gpu = cv2.UMat(full_frame)

        # Resize on GPU
        detection_frame_gpu = cv2.resize(full_frame_gpu, DETECTION_RES)

        # You can choose whether to bring it back to CPU here
        detection_frame = detection_frame_gpu.get()

        # detection_frame = cv2.resize(full_frame, DETECTION_RES)
        frame_queue.put((full_frame_gpu.get(), detection_frame))

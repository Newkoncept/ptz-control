...
while True:
    ...
    # print(f"VideoCapture opened: resolution={width}x{height}, FPS={fps}")
    # print(f"NDI sending frame size: {bgra_frame.shape[1]}x{bgra_frame.shape[0]}")
    # print(f"{canvas_height}, {canvas_width}")
    # print("")
    # print("")
    # print("")
    ...
    if conf_parameters["show_tracker"] and face_found and last_face_box:
        # Draw face box
        # print(last_face_box)

        bx1, by1, bx2, by2, bconf = last_face_box

        # Map box coords to zoomed frame coordinates
        box_x1_rel = bx1 - x1
        box_y1_rel = by1 - y1
        box_x2_rel = bx2 - x1
        box_y2_rel = by2 - y1

        scale_x = canvas_width / (x2 - x1)
        scale_y = canvas_height / (y2 - y1)

        box_x1_zoomed = int(box_x1_rel * scale_x)
        box_y1_zoomed = int(box_y1_rel * scale_y)
        box_x2_zoomed = int(box_x2_rel * scale_x)
        box_y2_zoomed = int(box_y2_rel * scale_y)

        # Draw box and confidence on preview frame
        cv2.rectangle(preview_frame, (box_x1_zoomed, box_y1_zoomed), (box_x2_zoomed, box_y2_zoomed), (0, 255, 0), 2)
        cv2.putText(preview_frame, f"{bconf:.2f}", (box_x1_zoomed, box_y1_zoomed - 10),
        ...
        # last_face_box = (x1, y1, x2, y2, conf)
        face_width = box_x2_zoomed - box_x1_zoomed
        face_height = box_y2_zoomed - box_y1_zoomed
        deadzone_width = int(face_width * conf_parameters["dead_zone_scale"])
        deadzone_height = int(face_height * conf_parameters["dead_zone_scale"])

        # deadzone_box_x1 = max(smoothed_cx - deadzone_width // 2, 0)
        # deadzone_box_y1 = max(smoothed_cy - deadzone_height // 2, 0)
        # deadzone_box_x2 = min(smoothed_cx + deadzone_width // 2, canvas_width)
        # deadzone_box_y2 = min(smoothed_cy + deadzone_height // 2, canvas_height)

        deadzone_box_x1 = int(box_x1_zoomed * conf_parameters["dead_zone_scale"])
        deadzone_box_y1 = int(box_y1_zoomed * conf_parameters["dead_zone_scale"])
        deadzone_box_x2 = int(box_x2_zoomed * conf_parameters["dead_zone_scale"])
        deadzone_box_y2 = int(box_y2_zoomed * conf_parameters["dead_zone_scale"])
        
        # Draw dead zone box
        # cv2.rectangle(preview_frame, (deadzone_x1, deadzone_y1), (deadzone_x2, deadzone_y2), (255, 0, 255), 2)
        # cv2.rectangle(preview_frame, (box_x1_zoomed, box_y1_zoomed), (box_x2_zoomed, box_y2_zoomed), (255, 0, 255), 2)
        cv2.rectangle(preview_frame, (deadzone_box_x1, deadzone_box_y1), (deadzone_box_x2, deadzone_box_y2), (255, 0, 255), 2)

    # print(f"Face box: {last_face_box}, Smoothed center: ({smoothed_cx}, {smoothed_cy})")
    # print(f"Dead zone coords: ({deadzone_x1}, {deadzone_y1}) to ({deadzone_x2}, {deadzone_y2})")
...
# "C:\Users\presi\AppData\Local\Programs\Python\Python310\python.exe"
# # Install python 3.10 from https://www.python.org/downloads/release/python-3100/
# C:\Users\presi\AppData\Local\Programs\Python\Python310\python.exe -m venv ndi-env
# cd ndi-env/Scripts
# activate
# pip install --upgrade pip
# pip install numpy ndi-python
